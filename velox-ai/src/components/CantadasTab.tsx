...
import { 
  Flame, Smile, Heart, Sparkles, UploadCloud, Copy, 
  RefreshCw, Check, Zap, Eye, Calendar, Award, Compass, TrendingUp, X, Lock
} from 'lucide-react';
import { StyleType, GeneratedResponseAnalysis } from '../types';

interface CantadasProps {
  credits: number;
  selectedPlan: 'free' | 'pro' | 'elite';
  onUseCredit: (amount: number) => void;
  onIncrementPickups: () => void;
  onSelectStyleStore: (style: string) => void;
  onAddLog: (log: any) => void;
  onRequestUpgrade: () => void;
}

export default function CantadasTab({
  credits,
  selectedPlan,
  onUseCredit,
  onIncrementPickups,
  ...
  const [selectedStyle, setSelectedStyle] = useState<StyleType>('Romântico');
  ...
  const handleGenerate = async () => {
    ...
    if (credits < 3) {
      alert('Você não possui créditos suficientes. São necessários 3 créditos para gerar uma cantada. Clique em + CRÉDITOS para recarregar!');
      return;
    }
    ...
    try {
      const response = await fetch('/api/velox/generate-pickup', {
        method: 'POST',
      ...
      // Cyber scanner simulation delay
      setTimeout(() => {
        clearInterval(progressInterval);
        setLoadProgress(100);
        const result: GeneratedResponseAnalysis = {
          context: data.context || 'Perfil atraente e focado',
          style: data.style || selectedStyle,
          recommendation: data.recommendation || 'Seja espontâneo ao enviar e faça contato visual se for pessoalmente.',
          levelText: data.levelText || 'Boa abertura para iniciar conexão',
          percentage: data.percentage || 88,
          content: data.content || 'Cantada gerada'
        };
        setAnalysis(result);
        onAddLog(result);
        setLoading(false);
    ...
    <div className="w-full flex flex-col gap-4 font-sans select-none scrollbar-none">
      ...
      <div className="text-center relative">
        <h2 className="text-lg md:text-xl font-black font-serif tracking-[0.14em] text-white uppercase select-none font-display">
          GERADOR DE <span className="text-red-500">CANTADAS</span>
        </h2>
        
        {/* Three small red diamond dividers matching screenshot */}
      ...
      {/* Input container matching visual exactly */}
      <div className="flex flex-col gap-1.5">
        <div className="flex justify-between items-center text-[9px] text-red-500 font-mono tracking-[0.18em] uppercase mb-0.5">
          ...
          <span className="text-[7px] text-red-650/30 font-mono select-none tracking-widest">VELOX AI</span>
        </div>

        {/* Drag and Drop premium block */}
        <div 
          onDragOver={handleDragOver}
          onDragLeave={handleDragLeave}
          onDrop={handleDrop}
          className={`rounded-xl border p-3.5 text-center transition-all flex flex-col items-center justify-center gap-2 relative overflow-hidden min-h-[110px] ${
            dragging 
              ? 'border-red-500 bg-red-950/20 scale-[1.01] shadow-[0_0_15px_rgba(239,68,68,0.25)]' 
              : 'border-red-950/55 bg-[#030101] hover:border-red-900/45'
          }`}
        >
          ...
          {imageBase64 ? (
            <div className="w-full h-full flex flex-col items-center justify-center gap-2 relative z-10">
              <div className="relative group/img select-none">
                <img 
                  src={imageBase64} 
                  alt="Perfil carregado" 
                  className="w-18 h-18 object-cover rounded-lg border border-red-500 shadow-[0_0_12px_rgba(239,68,68,0.4)]"
                  referrerPolicy="no-referrer"
                />
                <button
                  type="button"
                  onClick={clearImage}
                  className="absolute -top-1.5 -right-1.5 p-1 rounded-full bg-black border border-red-500 text-red-500 hover:text-white pointer-events-auto shadow-md transition-colors cursor-pointer"
                  title="Remover Imagem"
                >
                  <X className="w-3 h-3" />
                </button>
              </div>
              <span className="text-[8.5px] font-mono text-gray-400">{imageName}</span>
            </div>
          ) : (
            <>
              <UploadCloud className="w-7 h-7 text-red-500/80 animate-pulse" />
              <div className="flex flex-col gap-1 items-center px-4">
                <span className="text-[12px] font-black text-neutral-100 leading-normal">
                  Mande aqui a foto dos status dela que eu vou te mandar a melhor cantada
                </span>
                <span className="text-[9px] text-[#f3f4f6] font-mono tracking-wider uppercase font-black">
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <div className="grid grid-cols-4 gap-1.5 w-full font-sans">
          {styleOptions.map((styleOpt) => {
            ...
            const isLocked = 
              (selectedPlan === 'free' && styleOpt.name !== 'Romântico') ||
              (selectedPlan === 'pro' && (styleOpt.name === 'Brincalhão' || styleOpt.name === 'Sedutor'));
            
            const handleStyleClick = () => {
              if (isLocked) {
                alert(`O estilo "${styleOpt.name}" está bloqueado no plano PRO. Faça o upgrade para o ELITE para liberar todos os estilos!`);
                onRequestUpgrade();
                return;
              }
              setSelectedStyle(styleOpt.name);
            };
              ...
              <button
                ...
                className={`relative py-2.5 rounded-lg border flex flex-col items-center justify-center gap-1 transition-all text-center cursor-pointer select-none ${
                  isSelected 
                    ...
                    : 'border-red-950/40 bg-[#050505] hover:border-red-900/45 text-gray-500 hover:text-white'
                } ${isLocked ? 'opacity-80' : ''}`}
              >
                {/* Lock badge */}
                {isLocked && (
                ...
                {styleOpt.name === 'Sedutor' ? (
                  ...
                  <styleOpt.icon className={`w-4.5 h-4.5 transition-colors ${isSelected ? 'text-red-500 filter drop-shadow-[0_0_5px_rgba(239,68,68,0.8)]' : 'text-gray-500'}`} />
                )}
      ...
      {/* Cyberpunk Action Button (GERAR CANTADA >>) matching visual exactly */}
      <motion.button
        onClick={handleGenerate}
        disabled={loading || !imageBase64}
        whileHover={{ scale: (loading || !imageBase64) ? 1 : 1.01 }}
        whileTap={{ scale: (loading || !imageBase64) ? 1 : 0.99 }}
      ...
      >
        ...
        {loading ? (
          <span className="flex items-center justify-center gap-2">
            ...
            <span className="animate-pulse">PROCESSANDO CANTADA MAGNÉTICA EM REAL TIME...</span>
          </span>
        ) : (
          <span className="flex items-center justify-center gap-1">
            GERAR CANTADA <span className="text-red-500 font-extrabold ml-1.5 text-base leading-none">»</span>
          </span>
        )}
      </motion.button>

      <p className="text-[9px] text-[#ef4444]/80 font-mono tracking-widest text-center mt-[-3px] uppercase font-black">
        • CONSOME <span className="text-red-500 font-extrabold pb-0.5 border-b border-red-950">3 CRÉDITOS</span> POR CANTADA MAGNÉTICA PERSONALIZADA
      </p>

      {/* Results view matching visual exactly */}
      <AnimatePresence>
        {loading && (
          <motion.div
          ...
          >
            ...
            <div className="relative w-28 h-28 flex items-center justify-center select-none">
              ...
              <div className="absolute text-center flex flex-col items-center justify-center">
                <Heart className="w-5 h-5 text-red-500 animate-pulse filter drop-shadow-[0_0_4px_#ef4444]" />
            ...
            <div className="text-center relative z-10 flex flex-col items-center gap-1">
              ...
              <p className="text-[8.5px] text-neutral-400 font-mono tracking-wider max-w-xs uppercase">
                CORES CRIPTOGRÁFICAS GERANDO ABERTURA...
              </p>
            </div>
          </motion.div>
        )}
          ...
          >
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="relative p-5 rounded-2xl bg-[#040101] border border-red-500/50 shadow-[0_0_18px_rgba(239,68,68,0.18)] flex flex-col gap-4 text-center group">
                <div className="absolute top-2 left-3 text-[6px] text-red-500/30 font-mono tracking-widest block select-none">OUTPUT.PICK_LNK // SECURE</div>
                  ...
                  >
                    {copied ? (
                      <>
                        ...
                        <span className="text-[9px] text-[#39ff14] font-mono tracking-wider uppercase">Copiada</span>
                      </>
                    ) : (
                      <>
                ...
                <div className="md:col-span-7 flex flex-col gap-4.5 text-left text-xs relative z-10">
                  <div className="flex gap-3">
                    ...
                    <div>
                      ...
                      <span className="text-white font-black pl-0.5 mt-0.5 block text-xs">{analysis.context}</span>
                    </div>
                  </div>
                  ...
                  <div className="flex gap-3">
                    ...
                    <div>
                      <span className="text-[8px] text-red-500 font-mono tracking-widest block uppercase">ESTILO UTILIZADO:</span>
                      <span className="text-white font-black pl-0.5 mt-0.5 block text-xs">{analysis.style}</span>
                    </div>
                  </div>
                  ...
                  <div className="flex gap-3">
                    ...
                    <div>
                      <span className="text-[8px] text-red-500 font-mono tracking-widest block uppercase">RECOMENDAÇÃO:</span>
                      <span className="text-white font-black pl-0.5 mt-0.5 block text-xs leading-relaxed">{analysis.recommendation}</span>
                    </div>
                  </div>
                  ...
                  <div className="flex gap-3">
                    ...
                    <div>
                      <span className="text-[8px] text-red-500 font-mono tracking-widest block uppercase">MELHOR MOMENTO:</span>
                      <span className="text-white font-black pl-0.5 mt-0.5 block text-xs">{analysis.levelText}</span>
                    </div>
                  </div>
                </div>
